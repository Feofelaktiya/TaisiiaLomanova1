...
public class CheckMainPageElements {
    ...
    public void checkMainPageElements() {
        ...
        // 2 Open browser and maximize it
        WebDriver driver = new ChromeDriver();
        driver.manage().window().maximize();

        // 3 Open test site by URL
        ...
        // 6 Assert User name in the left-top side of screen that user is loggined
        assertEquals(driver.findElement(By.id("user-name")).getText(),
                "PITER CHAILOVSKII");

        // 7 Assert Browser title
        assertEquals(driver.getTitle(), "Home Page");

        // 8 Assert that there are 4 items on the header section are displayed and they have proper texts
        ...
        // 9 Assert that there are 4 images on the Index Page and they are displayed
        List<WebElement> icons = driver.findElements(By.cssSelector(".benefit-icon"));
        for (WebElement icon : icons) {
            assertTrue(icon.isDisplayed());
        }

        // 10 Assert that there are 4 texts on the Index Page under icons and they have proper text
        ...
        // 11 Assert a text of the main headers
        ...
        // 12 Assert that there is the iframe in the center of page
        ...
        // 13 Switch to the iframe and check that there is Epam logo in the left top conner of iframe
        WebElement iframeLogo = driver.findElement(By.cssSelector(".epam-logo"));
        assertTrue(iframeLogo.isDisplayed());

        // 14 Switch to original window back
